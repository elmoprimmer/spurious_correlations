...
import torch
from pxp.utils import one_hot_max, one_hot, one_hot_least, ModelLayerUtils
...
class LatentRelevanceAttributor:
    def __init__(self, layers_list_to_track, least_relevant_first=True) -> None:
        """
        Constructor
        ...
        self.latent_output = {}
        self.least_relevant_first = least_relevant_first
    ...
    def lrp_pass(self, model, inputs, targets, composite, initial_relevance, device):
        ...
        if self.least_relevant_first:
            ...
            if initial_relevance == 1:
                initial_relevance_function = one_hot_least
            elif initial_relevance == "logit":
                raise NotImplementedError(
                    "initial_relevance='logit' is not supported yet for least relevant last."
                )
...
class ZennitLatentRelevanceAttributor(LatentRelevanceAttributor):
    def __init__(self, layers_list_to_track, least_relevant_first=True) -> None:
        ...
        Args:
            layers_list_to_track (list): list of types of layers to track
        """
        super().__init__(layers_list_to_track, least_relevant_first=least_relevant_first)
        self.latent_output = {}
...
class ComponentAttibution:
    def __init__(self, attribution_type, model_type, target_layer_type, least_relevant_first=True):
    ...
    def attribute(
        self, model, dataloader, attribution_composite, abs_flag=True, Zplus_flag = False, device="cpu"
    ):
        model.eval()
        ...
        attributor = self.attributor(
            self.layer_names,
            least_relevant_first = self.least_relevant_first)
        ...
        for images, labels, _ in dataloader:
            ...
            for layer_name in self.layer_names:
                ...
                if Zplus_flag:
                    latent_relevance = torch.relu(latent_relevance)
