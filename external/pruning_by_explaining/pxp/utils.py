...
def one_hot_least(output, targets):
    """
    Get the one-hot encoded at the original indices in dim=1, except that
    we add one to the targets tensor and mod by number of classes
    so we get the wrong prediction for a 2 dim output.
    ...
    shifted_targets = (targets + 1) % num_classes
    return eye_matrix[shifted_targets]
