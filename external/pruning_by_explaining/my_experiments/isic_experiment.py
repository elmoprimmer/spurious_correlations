"""
Waterbirds pruning experiment (converted from notebook)
- SLURM-friendly CLI (argparse)
- Pruning + evaluation across prune_r values
- Saves results, figures, and a *separate model per r* with r in the filename

Modifications per request:
1) Linear pruning: weights are zeroed via pruner and saved directly (per r).
   Softmax pruning: use parse_head_mask + zero_vit_heads to bake pruning into a copy and save (per r).
2) groups=None is passed directly to get_sample_indices_for_group (function handles None).
3) Experiment folder name format: wb_<layer_type>_g-<groups or None>_n-<n_indices>_seed-<seed>
4) Save each rate as its own model, with r in the filename.
"""
...
def parse_args():
    ...
    parser.add_argument("--checkpoint", type=str, default="/home/primmere/ide/dfr/logs/vit_waterbirds.pth",
...
def _make_experiment_folder(args):
    g = _parse_groups(args.groups)
    g_str = "None" if g is None else "_".join(map(str, g))
    name = f"f_experiments/wb_{args.layer_type}_g-{g_str}_n-{args.n_indices}_seed-{args.seed}"
...
def main():
    ...
    # Experiment folder
    exp_dir = _make_experiment_folder(args)
        ...
        try:
            if plot_r_accuracy_lines is not None:
                ...
                plot_r_accuracy_lines(accs, save_path=os.path.join(exp_dir, f"acc_{args.layer_type.lower()}.png"),
                                      title=f"{args.layer_type} Layers of Waterbirds Model")
                plt.close()
            else:
