...
import utils
from vit_wdpruning import VisionTransformerWithWDPruning, _cfg, _cfg2, checkpoint_filter_fn
import math
...
def get_args_parser():
    ...
    parser.add_argument('--data-path', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR100', 'CIFAR10', 'IMNET', 'INAT', 'INAT19', 'wb', 'isic'],
...
def adjust_learning_rate(param_groups, init_lr, min_lr, step, max_step, warming_up_step=2, warmup_pruner=False,
                         init_lr_p=0.01):
...
def main(args):
    ...
    print(f"[rank {utils.get_rank()}] | train-set size = {len(dataset_train)}")
    ...
    indices = list(sampler_train)
    print(f"[rank {global_rank}] sampler length = {len(indices)}")
    ...
    elif args.arch == 'vit_b_16_wb':
        embed_dim = 768
        num_heads = 12
        model_path = '/home/primmere/ide/dfr/logs/vit_waterbirds_timm.pth'
    elif args.arch == 'vit_b_16_isic':
        embed_dim = 768
        num_heads = 12
        model_path = '/home/primmere/logs/isic_logs_4/vit_isic_v2_timm.pt'
    ...
    model = VisionTransformerWithWDPruning(num_classes=args.nb_classes,
                                           ...
                                           mlp_ratio=4, qkv_bias=True, distilled=args.distill,
    ...
    elif 'wb' == args.data_set:
        print("dataset = wb")
        checkpoint = torch.load(model_path, map_location="cpu")
        ckpt = checkpoint_filter_fn(checkpoint, model)
        model.default_cfg = _cfg2()
        missing_keys, unexpected_keys = model.load_state_dict(ckpt, strict=False)
        print('# missing keys=', missing_keys)
        print('# unexpected keys=', unexpected_keys)
        print('sucessfully loaded from pre-trained weights:', model_path)

    elif 'isic' == args.data_set:
        print("dataset = isic")
        checkpoint = torch.load(model_path, map_location="cpu")
        ckpt = checkpoint_filter_fn(checkpoint, model)
        model.default_cfg = _cfg2()
