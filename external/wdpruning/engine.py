...
def evaluate_classifiers(data_loader, model, device, classifiers=[6, 8, 10]):
    ...
    for data in metric_logger.log_every(data_loader, 10, header):
        ...
        for i in range(len(accuracy_classifiers)):
            metric_logger.meters['acc1_{}'.format(classifiers[i])].update(accuracy_classifiers[i][0].item(),
                                                                          n=batch_size)
            metric_logger.meters['acc5_{}'.format(classifiers[i])].update(accuracy_classifiers[i][1].item(),
                                                                          n=batch_size)
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print('* Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} loss {losses.global_avg:.3f}'
