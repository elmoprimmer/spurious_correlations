...
def build_dataset(is_train, args, infer_no_resize=False):
    ...
    if args.data_set == 'CIFAR100':
        ...
        dataset = INatDataset(args.data_path, train=is_train, year=2019,
                              category=args.inat_category, transform=transform)
        nb_classes = dataset.nb_classes
    elif args.data_set == 'wb':
        #To train with full train split:
        #split = 'train' if is_train else 'test'
        #to train with val data:
        split = 'val' if is_train else 'test'
        dataset = WaterBirdsDataset(args.data_path, split, transform=transform)
        nb_classes = 2
        ...
        #to train with val data:
        split = 'val' if is_train else 'test'
        dataset = ISICDataset(basedir=args.data_path, csv_file=args.metadata, split=split, transform=transform)
        nb_classes = 2
